...
import 'live_mode_state_test.mocks.dart';

void main() {
  late LiveModeState liveModeState;
  ...
  setUp(() {
    ...
    liveModeState = LiveModeState(context);
    mockStateMachine = MockAntennaStateMachine();
  });

  group('LiveModeState', () {
    test('should have the correct name', () {
      expect(liveModeState.name, equals('Live Mode'));
    });

    test('handleCommandReceived doesnt do anything', () {
      // Arrange
      final command = MockCommand();
      // Act
      expect(() => liveModeState.handleCommandReceived(command, mockStateMachine), returnsNormally);
    });
  });
}
